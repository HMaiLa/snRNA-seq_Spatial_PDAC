...
metadata <- tuft.merged@meta.data %>% as.data.frame() #Creating a data frame called metadata
...
cluster1 <- ctest[[1]]$table %>% mutate(cluster = 1)
cluster1$gene <- rownames(cluster1)
c1 <- dplyr::filter(cluster1, abs(logFC) >= 1, PValue < 0.05) # to filter out genes
cluster2 <- ctest[[2]]$table %>% mutate(cluster = 2)
cluster2$gene <- rownames(cluster2)
c2 <- dplyr::filter(cluster2, abs(logFC) >= 1, PValue < 0.05) 
cluster3 <- ctest[[3]]$table %>% mutate(cluster = 3)
cluster3$gene <- rownames(cluster3)
c3 <- dplyr::filter(cluster3, abs(logFC) >= 1, PValue < 0.05) 
cluster4 <- ctest[[4]]$table %>% mutate(cluster = 4)
cluster4$gene <- rownames(cluster4)
c4 <- dplyr::filter(cluster4, abs(logFC) >= 1, PValue < 0.05) 
cluster5 <- ctest[[5]]$table %>% mutate(cluster = 5)
cluster5$gene <- rownames(cluster5)
c5 <- dplyr::filter(cluster5, abs(logFC) >= 1, PValue < 0.05) 

adm.cellnames.pseudobulk <- bind_rows(cluster1, cluster2, cluster3, cluster4, cluster5)
...
# Fig2a
DefaultAssay(ADM) <- "RNA"
VlnPlot(ADM, features = c("CPB1","CPA1","REG1A",'EGF',#'CXCL2', #"MECOM","OLFM4",
                          "CFTR",'SLC4A4', #"DCDC2",
                          "MUC5B",'MUC6', 
                          #"BMX","SH2D6",
...
DotPlot(ADM.Pat, features = features) + RotatedAxis() + scale_color_gradient2(low="darkblue", mid="white", high="darkred") +labs(title="Expression of marker genes derived from our previous analysis")

# Fig 2g
genes <- c("MDK","IL15","IL19",
           "IL17RB","IL13RA1","IL6R", #"IL27RA","IL17RE",
           #"VEGFA","FGF12",#"PTPRC", # only specific to P2221
           "HCK",
           "PVR",#"NECTIN1","NECTIN2","NECTIN4",
           "LGALS3","LGALS4","LGALS9")
