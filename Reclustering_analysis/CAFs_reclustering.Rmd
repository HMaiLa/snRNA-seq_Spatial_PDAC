...
#"#67B39D" ,"#E7D59B") "#76E74A" "#6E33E0" ,"#B8C9E4" ,"#7AE07E" 
...
fib.only <- sts.non.malignant %>% subset(subset = Sample %in% sample.high.fib & 
                             Lv.2.Anno == "CAFs") # to make sure our population of CAFs is as pure as possible
...
# SCTRANFORM
fib.only <- SplitObject(fib.only, split.by = "Sample") #we split by Sample instead of Patient because some samples derived from same patient are sequenced at different batch
...
DimPlot(fib.only.simspec_css.sample.sct, group.by = "SCT_snn_res.0.1", label = T) + DimPlot(fib.only.simspec_css.sample.sct, group.by = "Lv.2.Anno",label = T)
...
metadata %>% mutate(Lv.2.Anno_condense = case_when(SCT_snn_res.0.1 %in% c("0","2") ~ "CAFs_MYO", # No ACTA2
                                          SCT_snn_res.0.1 == "4" ~ "Vascular smooth muscle", #check
                                          SCT_snn_res.0.1 == "3" ~ "CAFs_CXCL14+",
                                          SCT_snn_res.0.1 == "1" ~ "CAFs_NRT", #check
                                          )) -> metadata
...
df <- df %>% filter(!is.na(features.plot)) # removing rows with NA
...
# Scoring all stromal subsets against known program 
```{r}
stromal <- list()
